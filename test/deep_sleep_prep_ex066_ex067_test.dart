...
void main() {
  group('EX066 Legs Up Wall — Tier 1 registration', () {
    ...
    test('every pose uses the approved v2 asset, never a substitute', () {
      for (final pose in def!.poses) {
        expect(pose.approvedAsset, contains('assets/glow_up/exercises/legs_up_wall/female/v2/EX066_F_'));
      }
      expect(def.holdPose.approvedAsset, contains('04_HOLD'));
    });

    test('entry plays PREPARE -> LOWER_AND_TURN -> LEGS_UP once over the approved 10s loopCycleSeconds (2+5+3), never repeating', () {
      final e = def!;
      ...
      for (final s in [10.0, 20.0, 30.0, 40.0, 54.9]) {
        expect(at(s).label, 'HOLD', reason: 'must not re-animate the entry at $s s');
  ...
  group('EX067 Reclined Butterfly — Tier 1 registration', () {
    ...
    test('every pose uses the approved v2 asset, never a substitute', () {
      for (final pose in def!.poses) {
        expect(pose.approvedAsset, contains('assets/glow_up/exercises/reclined_butterfly/female/v2/EX067_F_'));
      }
    });

    test('entry plays PREPARE -> OPEN_KNEES -> SETTLE once over the approved 11s loopCycleSeconds (3+5+3), never repeating', () {
      final e = def!;
      PoseDefinition at(double s) => currentPoseFor(e, s);
    ...
    test('the knees are never animated being forced down — CLOSE_KNEES is a slow, hands-assisted close, and no instruction ever tells the user to force/push the knees down', () {
      final e = def!;
  ...
  group('holdAfterSetup engine fix does not regress existing single-frame-exit exercises', () {
    test('Tree Pose (EX059) still shows RESET for exactly the final 1 second', () {
    ...
    test('Savasana (EX060) still shows RECOVERY for exactly the final 1 second', () {
  ...
  group('deep-sleep-prep Workout wiring', () {
    final workout = kWorkoutCatalog.firstWhere((w) => w.id == 'deep-sleep-prep');

    test('order is Legs Up Wall, Reclined Butterfly, Neck Release, Box Breathing', () {
      expect(workout.exercises.map((e) => e.title).toList(), ['Legs Up Wall', 'Reclined Butterfly', 'Neck Release', 'Box Breathing']);
      expect(workout.exercises.map((e) => e.catalogId).toList(), ['EX066', 'EX067', 'EX034', 'EX035']);
    });
    ...
    test('Neck Release / Box Breathing keep their existing EX034/EX035 ids — no new id assigned', () {
      expect(workout.exercises[2].catalogId, 'EX034');
      expect(workout.exercises[3].catalogId, 'EX035');
    });
  });

  group('scope protection: EX001-EX065 untouched', () {
    test('Bedtime Meditation is still exactly EX001, EX061-EX065 in order', () {
      final bedtime = kWorkoutCatalog.firstWhere((w) => w.id == 'bedtime-meditation');
      expect(bedtime.exercises.map((e) => e.catalogId).toList(), ['EX001', 'EX061', 'EX062', 'EX063', 'EX064', 'EX065']);
    });

    test('EX032/EX033 legacy placeholders still exist in the Tier 2 catalog, untouched', () {
  ...
  group('AI Coach reflection config', () {
    test('EX066 prompt and signals match the approved spec', () {
      ...
      expect(config.prompt, 'How did Legs Up Wall feel? Note hamstring tension, back comfort, tingling or dizziness.');
    ...
    test('EX067 prompt and signals match the approved spec', () {
      ...
      expect(config.prompt, 'How did Reclined Butterfly feel? Note hip, groin, knee or lower-back comfort and whether you needed support.');
      expect(config.signals.map((s) => s.key).toList(), [
        'hip_comfort',
        'groin_comfort',
        'knee_comfort',
  ...
  group('manifest.json on-disk correctness (asset metadata, not read by app code)', () {
    test('legs_up_wall manifest matches the approved frames/timing/signals', () {
      final file = File('assets/glow_up/exercises/legs_up_wall/female/v2/manifest.json');
      ...
      expect((manifest['aiCoachNote']['trainingSignals'] as List), contains('numbness'));
    });

    test('reclined_butterfly manifest matches the approved frames/timing/signals', () {
      final file = File('assets/glow_up/exercises/reclined_butterfly/female/v2/manifest.json');
      expect(file.existsSync(), isTrue);
      final manifest = jsonDecode(file.readAsStringSync()) as Map<String, dynamic>;
    ...
    test('all six approved PNGs exist on disk for both exercises', () {
      for (final dir in ['legs_up_wall', 'reclined_butterfly']) {
        ...
        final pngs = folder.listSync().whereType<File>().where((f) => f.path.endsWith('.png'));
        expect(pngs.length, 6, reason: '$dir must have exactly 6 approved frames');
      }
    });
  });
}
